...
- [ ] **Step 3: Rewrite README and public docs.** Update remaining namespace literals in the exact listed Markdown consumers, put the exact Spec §4 disclaimer first, make synthetic KG/AQR, contracts, flow, reproducibility command, benchmark methodology, limitations/future experiments, then federated ERP secondary, and replace ownership, certification, production, superiority, and hallucination guarantees with evidence-linked or proposed wording.
