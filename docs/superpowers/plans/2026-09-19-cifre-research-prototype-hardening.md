...
- [ ] **Step 3: Verify the handoff scope.** Run `pytest tests/unit/test_documentation_contract.py::test_research_handoff_contract_and_links -q`; expected: PASS for section/disclaimer/link shape while exact metric fields remain explicitly pending T13.
...
- [ ] **Step 4: Generate, use, and remove temporary final metrics in one shell.** Keep the following as one shell block; the documentation copy/use and cleanup must happen before the block exits:
  ...
  # future commit, then run the focused documentation test without artifact
  # assertions.
  pytest tests/unit/test_documentation_contract.py -q
  git diff --check
  rm -rf -- "$cifre_tmp_dir"
